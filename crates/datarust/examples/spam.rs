...
use datarust::metrics::classification::{accuracy_score, precision_score, recall_score, f1_score};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _ in 0..n {
        ...
        for j in 0..n_features {
            ...
            let freq = if is_spam == 1.0 && j < 10 {
                base_freq + rng.next_f64() * 0.5
            } else if is_spam == 0.0 && j >= 10 && j < 20 {
                base_freq + rng.next_f64() * 0.5
            } else {
                base_freq
            };
    ...
    println!("Synthetic TF-IDF Data: {} samples, {} features", x.nrows(), x.ncols());
    
    let spam_count = y.iter().filter(|&&v| v == 1.0).count();
    println!("Class distribution: {} Spam, {} Ham", spam_count, n - spam_count);
